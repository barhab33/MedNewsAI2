 ...
 *
 * Full pipeline with hard cap of 10 articles:
 *  - Crawl feeds → score relevance → take top 10
 *  - Summarize top 10 via Gemini (if key provided)
 *  - Discover image (OG/Twitter → Pexels fallback)
 *  - Insert new ones
 *  - PRUNE: keep only newest 10 in `medical_news` (delete the rest)
 */
...
const MAX_ITEMS_PER_FEED = 20; // fetch a bit more so ranking has choices
const MAX_CANDIDATES = 60;     // hard cap across feeds (before ranking)
const TAKE_TOP = 10;           // <-- you asked for exactly 10
...
const firstTag = (chunk, tag) => (textBetween(chunk, tag)[0] || "").trim();
const tryAttrLink = (chunk) => (chunk.match(/<link[^>]*\bhref="([^"]+)"/i) || [])[1] || "";
...
function relevanceScore({ title, description, url, pubDate }) {
  ...
  // prefer canonical over Google News redirects
  ...
  if (ts) {
    ...
    score += Math.max(0, 7 - days); // up to +7 for fresh items
  }
...
async function rankAndSelectTop(rawItems) {
  ...
  const scored = uniq.map(it => ({
    ...it,
    score: relevanceScore(it)
  }));

  // sort by score desc, then by pubDate desc
  scored.sort((a, b) => {
    if (b.score !== a.score) return b.score - a.score;
    return parseDate(b.pubDate) - parseDate(a.pubDate);
  });

  return scored.slice(0, TAKE_TOP);
}
...
Write 2–3 short bullet points (plain text; no markdown bullets) covering the core finding, context, and any caveats.
...
// ------------------ Supabase helpers ------------------
async function tableHasUrl(url) {
  const { data, error } = await supabase.from("medical_news").select("id").eq("url", url).limit(1);
  if (error) { console.error("existsByUrl error:", error); return false; }
...
async function pruneToNewest(limit = 10) {
  ...
  if (delErr) {
    console.error("prune delete error:", delErr);
  } else {
    console.log(`Pruned table to newest ${keepIds.length} rows (ordered by ${orderCol}).`);
  }
}

// ------------------ Main ------------------
async function main() {
  const mode = process.env.SUPABASE_SERVICE_ROLE ? "service-role" : "anon";
  console.log(`Crawler start (auth: ${mode}) • Target top ${TAKE_TOP}`);

  // 1) Gather candidates
  const raw = await gatherCandidates();
  console.log(`Fetched ${raw.length} candidate items from ${FEEDS.length} feeds`);

  // 2) Rank & take top N
  const top = await rankAndSelectTop(raw);
  console.log(`Selected top ${top.length} by relevance`);

  // 3) For each of the top N: dedupe vs DB, summarize, image, insert
  let inserted = 0;
  for (const it of top) {
    try {
      if (!it.url) continue;
      if (await tableHasUrl(it.url)) {
        console.log(`= Exists, skip: ${it.title.slice(0, 80)}`);
        continue;
      }

      // Summarize only these top 10
      ...
      if (ok) {
        inserted++;
        console.log(`+ Inserted: ${row.title.slice(0, 100)} — ${row.url}`);
      }
      await sleep(120);
    } catch (e) {
  ...
  // 4) PRUNE: keep only newest TAKE_TOP rows
  await pruneToNewest(TAKE_TOP);
